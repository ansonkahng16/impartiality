...
vanilla = pickle.load(open('p2_long_run_with_bucket_versus_base.pickle', 'rb'))  # not actually vanilla; this is base
# vanilla = pickle.load(open('test2_long_run_with_bucket_versus_vanilla.pickle', 'rb'))
	...
	for p in test_p:
		for m in test_m:  # with a fixed l, p, m
			if (m,l) in test_m_l:
				...
				filename = 'k_figs_dave/k_l={}_p={}_m={}.png'.format(l,int(p*10),m)
				print(filename)
...
for l in test_l:
	for p in test_p:
		for k in test_k:  # with a fixed l, p, k
			...
			filename = 'm_figs_dave/m_l={}_p={}_k={}.png'.format(l,int(p*10),k)
			print(filename)
