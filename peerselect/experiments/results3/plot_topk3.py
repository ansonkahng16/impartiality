...
test_n = [120]
test_k = [10, 24, 30, 40, 60] #[10]
test_m = [60] #[10, 30, 40, 60]
test_l = [4]
test_p = [0, 0.1, 0.5, 0.8, 0.9, 1.0]
mechanisms = ["Vanilla", "ExactDollarPartition", "Partition", "DollarPartitionRaffle", "DollarRaffle", "CredibleSubset", "Bucket", "TopK"]
...
for l in test_l:
	for p in test_p:
		for m in test_m:  # with a fixed l, p, m
			...
			tk_plot = ax.bar(ind + 7*width, tmp[7], width, color='black')
			...
			# ax.legend((v_plot[0], edp_plot[0], p_plot[0], dpr_plot[0], dr_plot[0], cs_plot[0], b_plot[0], tk_plot[0]), ("Vanilla", "ExactDollarPartition", "Partition", "DollarPartitionRaffle", "DollarRaffle", "CredibleSubset", "Bucket", "TopK"), prop=fontP, bbox_to_anchor=(1.25,1.05), ncol=1)

			filename = 'k_figs_topk_lim/k_l={}_p={}_m={}.png'.format(l,int(p*10),m)
			print(filename)
...
# 			filename = 'm_figs_topk_lim/m_l={}_p={}_k={}.png'.format(l,int(p*10),k)
# 			print(filename)
